function [vTh,vR] = zern_pos2pol(mPos,vCenter,radius)
% function [vTh,vR] = zern_pos2pol(mPos,vCenter,radius)
%
% Calculate angles and normalized radii of points specified in
% cartesian coordinates
%
% vTh     : array with angles (defined relative to x, counter clockwise positive)
% vR      : array with normalized radii
%
% mPos    : [x,y] point coordinates (one set per row)
...
% check input
  ...
  if isempty(mPos), vTh = []; vR = []; return; end

  if size(mPos,2) ~= 2
    error(' Each row of mPos should contain the [x,y] coordinates of a point');
  end
  
  if numel(vCenter) ~= 2
    error(' vCenter should contain the [x,y] center of the Zernike normalization disk');
  end
  
  if numel(radius) ~= 1
    error(' radius should contain the radius of the Zernike normalization disk');
  end

% generate angle and radius images

  [vTh,vR] = cart2pol((mPos(:,1)-vCenter(1)),...
                      (mPos(:,2)-vCenter(2)));
