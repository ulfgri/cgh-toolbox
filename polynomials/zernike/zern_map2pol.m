function [mTh,mR] = zern_map2pol(vSize,vCenter,radius,vPixSep)
% function [mTh,mR] = zern_map2pol(vSize,vCenter,radius,vPixSep)
%
% Calculates angles and normalized radii for each element of an image array
%
% mTh     : array with angles at each element of the image array
%           (defined relative to x, counter clockwise positive)
% mR      : array with normalized radii at each element of the image array
%
% vSize   : [row,column] size of the image array
...
% check input parameters
  ...
  if numel(vSize) ~= 2
    error(' vSize should contain the [row,column] size of the image array');
  end

% obtain [X,Y] position of each image pixel

  [mPos(:,1),mPos(:,2)] = ind2sub(vSize,1:prod(vSize));
  mPos = sub2pos(mPos,vSize,vPixSep);
  
% obtain angle and radius  

  [mTh,mR] = zern_pos2pol(mPos,vCenter,radius);
  
% reshape output arrays

  mTh = reshape(mTh,vSize);
  mR = reshape(mR,vSize);  
