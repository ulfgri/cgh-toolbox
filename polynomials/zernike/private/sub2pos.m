function [mPos] = sub2pos(mij,vSize,vPixSep)
% function [mPos] = sub2pos(mij,vSize,vPixSep)
%
% User defined function that converts an array index [i,j] into
% a position [x,y]
%
% mPos    : [x,y] coordinates of pixels
%
% mij     : [i,j] array indices of pixels
...
% set defaults
  ...
  if nargin < 3, vPixSep = []; end
  if isempty(vPixSep), vPixSep = 1; end
  if numel(vPixSep) == 1, vPixSep = [vPixSep,vPixSep]; end
  
  if isempty(mij), mPos = []; return; end
  
% check input  
  
  if size(mij,2) ~= 2
    error(' Each row of mij should contain the [i,j] image array indices of a pixel');
  end

% display message on first use (delete this statement when edit is complete)

%  warning([' Edit function sub2pos for the desired conversion of image array ',...
%           'indices [i,j] to pixel positions [x,y]']);
  
% define the conversion here

% example 1:

%  mPos(:,1) = mij(:,1).*vPixSep(1);
%  mPos(:,2) = mij(:,2).*vPixSep(2);  
  
% example 2:  

  mPos(:,1) = mij(:,2).*vPixSep(1);
  mPos(:,2) = (vSize(1)-mij(:,2)+1).*vPixSep(2);
