function [eixy,pval,kval,nedg] = isophase_edges(fphase,par,tile,vphase,tol,grid,verbose)
%function [eixy,pval,kval,nedg] = isophase_edges(fphase,par,tile,vphase,tol,grid,verbose)
%
...
% Initial version: Ulf Griesmann, NIST, September 2018
    ...
    for ke = 1:4
        ...
        if verbose
            fprintf('     %d isophase - edge %d crossings\n', size(xy,1),ke);
        end
    ...
    % the algorithm that detects isophase-edge intersections can also return 
    ...
    [px,py] = vphderiv(fphase,par,eixy,tol.hderiv);
    idz = (px == 0) & (py == 0);
    if any(idz)
        eixy = eixy(~idz,:);
        pval = pval(~idz);
